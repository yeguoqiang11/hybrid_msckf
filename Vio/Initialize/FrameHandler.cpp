...
bool FrameHandler::TryInitialize() {
    ...
    if (state_ == InitTrackState::INITIALIZED && !imuInitializing_.IsSuccess() ){
        ...
        bool flag = imuInitializing_.AligmentNew(doStereo_);
        if (flag) {
            if(imuInitializing_.IsSuccess()) {
                ...
                Eigen::Vector3d GIj = leftCam_->mR_S_c * Gcj;
                Eigen::Quaterniond q_Ij_g = MathUtil::GetQfromA(GIj);
                Eigen::Vector3d Vgj = q_Ij_g.matrix() * leftCam_->mR_S_c * imuStateJ.Rwc * imuStateJ.Vj;
...
void FrameHandler::CalcStereo(const cv::Mat &Ir) {
    ...
    // Stereo pose
    Eigen::Matrix3d Rrl = rightCam_->mR_C_s * leftCam_->mR_S_c;
    Eigen::Vector3d prl = rightCam_->mp_C_s + rightCam_->mR_C_s * leftCam_->mp_S_c;
    ...
    const double minDepth = 1.0 / leftCam_->mmax_inverse_depth;
    const double maxDepth = 1.0 / leftCam_->mmin_inverse_depth;
...
void FrameHandler::SaveSlamPose(const std::string &path) {
    ...
    if (ofs.is_open()) {
        for (size_t i = 0; i < frameDataset_.size(); i++) {
            const cv::Mat R = frameDataset_[i]->R();
            const cv::Mat t = frameDataset_[i]->T();
            ofs << std::fixed << std::setprecision(6) << frameDataset_[i]->Time()
                << "," << R.at<double>(0, 0) << "," << R.at<double>(0, 1) << "," << R.at<double>(0, 2)
                << "," << R.at<double>(1, 0) << "," << R.at<double>(1, 1) << "," << R.at<double>(1, 2)
                << "," << R.at<double>(2, 0) << "," << R.at<double>(2, 1) << "," << R.at<double>(2, 2)
                << "," << t.at<double>(1, 0)<< ","  << t.at<double>(1, 0)<< ","  << t.at<double>(2, 0) << "\n";
        }
...
void FrameHandler::DebugShow(int showType) {
    ...
    if(showType == 2) {
        cv::Mat visImg;
        cv::cvtColor(currFrame_->Image(), visImg, cv::COLOR_GRAY2BGR);
        for (const auto &feature : currFrame_->Features()) {
            cv::circle(visImg, feature.pt, 3, cv::Scalar(0,0,255), -1);
        }
