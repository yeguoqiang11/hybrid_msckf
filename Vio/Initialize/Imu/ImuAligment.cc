...
bool VisualInertialAligment::AligmentNew(bool doStereo) {
    ...
    if (vel_set.size() != state_dataset_.size()) {
        std::cout << "error: vel size != state size!!!" << std::endl;
        return false;
    }
    ...
    std::unordered_map<int, double *> vel;
    ...
    for (size_t i = 0; i < static_cast<size_t>(scaleSize); i++) {
        ...
        for (int j = 0; j < static_cast<int>(vel_set.size()); j++) {
            vel[j] = new double[3]{vel_set[j](0), vel_set[j](1), vel_set[j](2)};
        }
        ...
        for (int n = 0; n < static_cast<int>(state_dataset_.size()) - 1; n++) {
            ...
            ceres::CostFunction *init_cost = new ImuInitCost(sqrt_info, Pi, Pj, Gw, tcb_, bgj, Rcb_, Rbiw, Rbjw, state_j.imu0);
            // ceres::CostFunction *aligment_cost = new AligmentCost(sqrt_info, Rcb, Rbiw, Rbjw, Pi, Pj, Gw, tcb, state_j.imu0);
            // ceres::CostFunction *aligmentExtr_cost = new AligmentExtrinsicCost(sqrt_info, state_i.Rwc.transpose(),
            //     state_j.Rwc.transpose(), Pi, Pj, Gw, state_j.imu0);
            // ceres::ResidualBlockId idx = problem.AddResidualBlock(aligmentExtr_cost, NULL, vel[n], vel[n + 1], bg, ba, rvec_gw, rv_cb, &scale);
            problem.AddResidualBlock(init_cost, NULL, vel[n], vel[n + 1], ba, rvec_gw, &scale);
            // ceres::ResidualBlockId idx = problem.AddResidualBlock(aligment_cost, NULL, vel[n], vel[n + 1], bg, ba, rvec_gw, &scale);
        }
        ...
        if (cost < min_cost) {
            ...
            // good_rcb << rv_cb[0], rv_cb[1], rv_cb[2];
            for (int m = 0; m < static_cast<int>(good_vel.size()); m++) {
                good_vel[m] = Eigen::Vector3d(vel[m][0], vel[m][1], vel[m][2]);
                delete[] vel[m];
            }
        }
    }
    if (true) {
        ...
        for (size_t i = 0; i < state_dataset_.size(); i++) {
            ...
            state_dataset_[i].Vj = good_vel[i] * scale_;
            state_dataset_[i].twc *= scale_;
            frame_dataset_[i].imu_state = state_dataset_[i];
        }

        if (min_cost < 8e5)is_success_ = true;
        std::cout << "min cost: " << min_cost << std::endl;
        std::cout << "rgw: " << good_rgw[0] << "," << good_rgw[1] << "," << good_rgw[2] << std::endl;
        std::cout << " MAP scale: " << scale_ << std::endl;
        std::cout << " MAP ba: " << ba_.transpose() << std::endl;
        std::cout << " MAP bg: " << bg_.transpose() << std::endl;
        std::cout << " MAP Gw: " << Gw_.transpose() << std::endl;
        std::cout << "----------------" << std::endl;
        // cv::waitKey(0);
    }
